...
async fn events_handler(auth: AuthCtx) -> Response {
    match auth.0 {
        Some(ref user) => {
            Json(serde_json::json!({ "events": [], "user_id": user.user_id, "roles": user.roles })).into_response()
        }
        None => (
            StatusCode::UNAUTHORIZED,
...
async fn main() {
    ...
    tracing::info!("clickstream-service — listening at /api/v1/events/*");
